...
namespace Fastie.Screens.Task
{
    public partial class DetailAssignTaskForm : Form
    {
        ...
        public DetailAssignTaskForm(AccountId accountId)
        {
            InitializeComponent();
        ...
        {
            List<Department> danhSachBoPhan = departmentBLL.GetDepartmentList();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            var task = new TaskInfo()
            {
                ...
                IdBoPhanGiaoViec = accountId.IdBoPhan.ToString(),
                Id = taskBLL.TaoCongViecId(taskBLL.LayIdLoaiCongViecTuTen(customComboBox1.Texts), accountId.IdBoPhan.ToString()),
                ...
                IdTaiKhoanGiaoViec = accountId.IdTaiKhoan.ToString(),
        ...
        private void LayQuanLiBoPhanDuocChon()
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                string idBoPhan = row.Cells["idBoPhan"].Value?.ToString();
                if (!string.IsNullOrEmpty(idBoPhan))
                {
                    // Get managers for each department in dgvDepartments
                    List<AcceptTaskPersonnel> departmentManagers = departmentBLL.LayQuanLiBoPhan(idBoPhan);
